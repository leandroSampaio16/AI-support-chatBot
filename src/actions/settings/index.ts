...
export const onIntegrateDomain = async (domain: string, icon: string) => {
  console.log('🔍 Starting onIntegrateDomain function...');
  ...
  try {
    ...
    console.log('✅ Domain exists:', domainExists ? 'Yes' : 'No');

    if (!domainExists) {
      console.log('🔍 Checking domain limit for plan:', subscription?.subscription?.plan);
      ...
      if (
        ...
        console.log('✅ User is eligible to add a domain, proceeding...');
        ...
        console.log('❌ User has reached the domain limit.');
        return {
          status: 400,
